...
* @brief callback of get frame
*/
typedef void (*FramePairCallback)(IFramePtr frameDep, IFramePtr frameRgb, void* usrData);
...
public:
	...
	**/
	...
	/**
	* @~chinese
	* @brief      打开数据流并通过回调函数返回帧数据，启流后等待相机出第1帧数据才返回
	* @param[in]  infoDep			：需要打开的深度流格式信息, 可由getStreamInfos返回
	* @param[in]  infoRgb			：需要打开的RGB流格式信息, 可由getStreamInfos返回
	* @param[in]  callback          : 返回帧数据的回调函数
	* @param[in]  userData          : 用户数据
	* @param[in]  timeouts          : 等待第1帧数据超时值,单位毫秒
	* @return     成功:SUCCESS, 失败:其他错误码
	* @~english
	* @brief      Start stream and return frame by callback
	* @param[in] infoDep      		：stream information of depth, returned by getStreamInfos
	* @param[in] infoRgb      		：stream information of RGB, returned by getStreamInfos
	* @param[in]  callback          : frame callback
	* @param[in]  userData          : the user data
	* @param[in]  timeouts          : the timeouts of wait first frmae recv after start stream, unit is milliseconds
	* @return success:return SUCCESS, fail:other error code
	**/
	virtual ERROR_CODE startStream(StreamInfo infoDep, StreamInfo infoRgb, FramePairCallback callback, void* userData, int timeouts) = 0;
	...
	* @~english
	* @brief      Start stream and return frame by callback
	* @param[out] infoDep      		：stream information of depth, returned by getStreamInfos
	* @param[out] infoRgb      		：stream information of RGB, returned by getStreamInfos
	* @param[in]  callback          : frame callback
	* @param[in]  userData          : the user data
	* @return success:return SUCCESS, fail:other error code
	**/
	virtual ERROR_CODE startStream(StreamInfo infoDep, StreamInfo infoRgb, FramePairCallback callback, void* userData) = 0;
	...
	* @return success:return SUCCESS, fail:other error code
	**/
	virtual ERROR_CODE startStream(STREAM_TYPE streamType, StreamInfo info, FrameCallback callback, void* userData, int timeouts) = 0;
	...
	* @return success:return SUCCESS, fail:other error code
	**/
	virtual ERROR_CODE startStream(STREAM_TYPE streamType, StreamInfo info, int timeouts) = 0;
	...
	* @return     成功:SUCCESS, 失败:其他错误码
	* @~english
	* @brief      Stop stream, all stream
	* @return success:return SUCCESS, fail:other error code
	**/
	virtual ERROR_CODE stopStream() = 0;
	...
	**/
	...
	/**
	* @~chinese
	* @brief      获取成对的深度和RGB帧及激光线扫IR组
	* @param[out] depthFrame		：返回深度帧
	* @param[out] rgbFrame			: 返回RGB帧
	* @param[out] lineIRGroup		: 返回激光线IR组帧
	* @param[in]  timeout_ms		：超时时间，单位为毫秒
	* @return     成功:SUCCESS, 失败:其他错误码
	* @~english
	* @brief      Get the paired frame of depth and rgb and laser line IR group
	* @param[in]  depthFrame		：return the paired depth frame
	* @param[out] frame				：return the paired rgb frame
	* @param[out] lineIRGroup		：return the frame group of laser line IR
	* @param[in]  timeout_ms		：timeout in millisecond
	* @return success:return SUCCESS, fail:other error code
	**/
	virtual ERROR_CODE getPairedFrame(IFramePtr& depthFrame, IFramePtr& rgbFrame, std::vector<IFramePtr>& lineIRGroup, int timeout_ms = 5000) = 0;
	...
	* @~chinese
	* @brief      获取IMU数据
	* @param[out] imu_data[]		：IMU数据组
	* @param[in out]  count		    ：获取的数据组数
	* @return     成功:SUCCESS, 失败:其他错误码
	* @~english
	* @brief      Get IMU calibration data
	* @param[out] imu_data[]		：IMU data array
	* @param[in out]  count		    ：The number of data groups obtained
	* @return success:return SUCCESS, fail:other error code
	**/
	virtual ERROR_CODE  getImuData(imu_data_t imu_data[], int& count) = 0;
	...
	* @return     成功:SUCCESS, 失败:其他错误码
	* @~english
	* @brief      Set face tag started
	* @return success:return SUCCESS, fail:other error code
	**/
	virtual ERROR_CODE setFaceFlag(bool& flag) = 0;
	...
	* @return     成功:SUCCESS, 失败:其他错误码
	* @~english
	* @brief      Get face tag started
	* @return success:return SUCCESS, fail:other error code
	**/
	virtual ERROR_CODE getFaceFlag(bool& flag) = 0;
	...
	* @return     成功:SUCCESS, 失败:其他错误码
	* @~english
	* @brief      hid device Interaction
	* @return success:return SUCCESS, fail:other error code
	**/
	virtual ERROR_CODE hidInteraction(char* incmd, char* out) = 0;

	/**
	* @~chinese
	* @brief      从hid设备 中下载文件（将src下载到store）
	* @param[in] srcPath		：来源路径
	* @param[in] storePath		：去向路径
	* @return     成功:SUCCESS, 失败:其他错误码
	* @~english
	* @brief      hid device Interaction
	* @return success:return SUCCESS, fail:other error code
	**/
	virtual ERROR_CODE hidLoadFile(const char* srcPath, const char* storePath) = 0;

	/**
	* @~chinese
	* @brief      上传文件到hid设备（将src传到store）
	* @param[in] srcPath		：来源路径
	* @param[in] storePath		：去向路径
	* @return     成功:SUCCESS, 失败:其他错误码
	* @~english
	...
	* @return success:return SUCCESS, fail:other error code
	**/
	virtual ERROR_CODE hidUploadFile(const char* srcPath, const char* storePath) = 0;
};
