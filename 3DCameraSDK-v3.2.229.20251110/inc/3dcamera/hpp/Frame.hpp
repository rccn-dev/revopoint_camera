...
public:
	...
	* @brief      设置标记点及点云数据
	* @~english
	* @brief      set markers and point clouds info
	**/
	virtual void setMarkersAndPointsInfo(std::vector<markerData>& markers, std::vector<pointData>& pointClouds) = 0;

	/**
	* @~chinese
	* @brief      设置标记点数据
	* @~english
	* @brief      set markers info
	**/
	virtual void setMarkersInfo(std::vector<markerData>& markers) = 0;

	/**
	* @~chinese
	* @brief      设置点云数据
	* @~english
	* @brief      set point clouds info
	**/
	virtual void setPointsInfo(std::vector<pointData>& pointClouds) = 0;

	/**
	* @~chinese
	* @brief      获取点云及标记点数据状态
	* @~english
	* @brief      get markers and point clouds state
	**/
	virtual bool getMarkersPCsProcState() = 0;

	/**
	* @~chinese
	* @brief      获取标记点数据,只在STREAM_FORMAT_IR_MARKERS格式有效
	* @~english
	* @brief      get markers data, only STREAM_FORMAT_IR_MARKERS valid
	**/
	virtual const std::vector<markerData>& getMarkersInfo() const = 0;

	/**
	* @~chinese
	* @brief      获取点云数据,只在STREAM_FORMAT_IR_MARKERS格式有效
	* @~english
	* @brief      get markers data, only STREAM_FORMAT_IR_MARKERS valid
	**/
	virtual const std::vector<pointData>& getPointCloudsInfo() const = 0;
};
/*@} */
}
#endif