...
* @param[in]	currentCameras	当前的相机列表
...
* @param[in]	currentCameras	the camera list of current
...
* @{
*/
typedef void (*CameraChangeCallback)(std::vector<CameraInfo>& currentCameras, std::vector<CameraInfo>& removedCameras, void * userData);
...
public:
	...
	* @return success:return SUCCESS, fail:other error code
	**/ 
    virtual ERROR_CODE setCameraChangeCallback(CameraChangeCallback callback, void *userData, bool isSync = false) = 0;
...
* @brief     设置SDK配置路径
* @~english
* @brief     Set sdk config path
**/
CS_API void setSdkConfigPath(const char* configPath);

/**
* @~chinese
* @brief     设置SDK配置数据
* @~english
* @brief     Set sdk config
**/
CS_API void setSdkConfigData(const char* config);
...
* @brief     Set sdk config
**/
CS_API void setRPCConfig(const char* ip, int port);
